...
const FlashcardAIGen = () => {
  ...
  return (
    <div className='p-5'>
      <button className='btn mb-5' onClick={click}>get dummy data</button>
      <form onSubmit={submit}>
        ...
        <input
          ...
          value={body.choices}
				/><br /><br />
        <button className='btn mb-5' onClick={click}>get custom data</button>
      </form>
      {data ? <div>{data.map((i, ii) => (
        <div key={ii}>
          <p className='font-bold'>{i.question}</p>
          <p className='text-sm'>{i.answer}</p>
        </div>
      ))}</div> : <p>loading or no data</p>}
      {status && <p>{status}</p>}
